...
int main() {
    ...
    // This simple lexer doesn't handle comments, so we'll strip them out.
    size_t pos;
    ...
    do {
        ...
        if (token.type != TokenType::UNKNOWN) { // Ignore unknown tokens
    ...
    std::string assembly;
    if (ast) { // Only generate code if parsing was successful
        try {
