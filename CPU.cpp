...
CPU::CPU(int memory_size, int stack_size) : memory(memory_size, 0), stack_base(memory_size - stack_size) {
    ...
    reg_B = 0;
    pc = 0;
    sp = memory_size - 1; // Stack grows downwards
    zero_flag = false;
    carry_flag = false;
}
...
uint8_t CPU::getValue(const std::string& arg) {
    ...
    // Assumes arg is a number for ldi, sta, lda etc.
...
void CPU::execute(const Instruction& instr) {
    ...
    // Data Transfer
    if (instr.opcode == "ldi") { // ldi r D
        uint8_t val = getValue(instr.arg2);
        if (instr.arg1 == "A") reg_A = val;
        else if (instr.arg1 == "B") reg_B = val;
    } else if (instr.opcode == "lda") { // lda D (alias for mov A M D)
        reg_A = memory[getValue(instr.arg1)];
    } else if (instr.opcode == "sta") { // sta D (alias for mov M A D)
    ...
    } else if (instr.opcode == "jne") { // jne is an alias for jnz
        if (!zero_flag) {
            next_pc = labels.at(instr.arg1);
        }
    }
...
// --- Assembly Parser (simplified for this CPU) ---
